...
export default function SalesScreen() {
  ...
  const { items, isLoading } = useSelector((s: RootState) => s.sales);
  ...
  const filtered = items.filter(s => {
    ...
    const matchesSearch = s.invoiceNumber.toLowerCase().includes(search.toLowerCase()) ||
      (s.partyName || s.party?.name || '').toLowerCase().includes(search.toLowerCase());
    if (!matchesSearch) return false;

    // 2. Status
    const isPaid = (s.dueAmount || 0) === 0;
  ...
  // Calculate totals from FILTERED items so the stats update dynamically
  ...
  const renderEmpty = () => (
    <View style={S.emptyBox}>
      ...
      <Text style={S.emptySub}>Tap + to record your first sale.</Text>
    </View>
  );

  const openPreview = (item: Sale) => {
    setPreviewInvoice({
      id:           item.id,
      type:         'SALE',
    ...
    <View style={{ flex: 1, backgroundColor: T.bg }}>
      ...
      <View style={[S.headerGrad, { paddingTop: insets.top + 16, backgroundColor: T.navy }]}>
        <View style={S.headerContent}>
          <View>
            ...
            <Text style={S.headerSub}>Manage your sales & invoices</Text>
          </View>
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={S.filterRow}>
          ...
          <TouchableOpacity style={[S.fChip, statusFilter==='PAID' && S.fChipActive]} onPress={()=>setStatusFilter('PAID')}>
            <Text style={[S.fTxt, statusFilter==='PAID' && S.fTxtActive]}>Paid</Text>
          </TouchableOpacity>
      ...
      <View style={S.statsRow}>
        ...
        <View style={[S.statCard, { borderLeftWidth: 1, borderLeftColor: T.bdrL }]}>
          ...
          <Text style={[S.statVal, { color: T.emerald, fontSize: 16 }]}>{fmt(totalRevenue)}</Text>
        </View>
        <View style={[S.statCard, { borderLeftWidth: 1, borderLeftColor: T.bdrL }]}>
            ...
            <TouchableOpacity style={S.card} activeOpacity={0.7} onPress={() => openPreview(item)}>
              <View style={S.cardTop}>
                ...
                <View style={S.infoWrap}>
                  <Text style={S.name} numberOfLines={1}>{item.partyName || item.party?.name || 'Walk-in Customer'}</Text>
                ...
                <View style={S.qtyWrap}>
                  ...
                  <Text style={[S.unit, isPaid ? { color: T.emerald } : { color: T.rose }]}>
                    {isPaid ? '✓ Paid' : `Due: ${fmt(item.dueAmount || 0)}`}
                  </Text>
                </View>
              </View>
              ...
              <View style={S.cardBot}>
                ...
                <TouchableOpacity style={S.actionBtn} onPress={() => apiService.openSalePdf(item.id)}>
                  <Text style={S.actionBtnTxt}>📄 PDF</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[S.actionBtn, { marginLeft: 8 }]} onPress={() => setEditingSale(item)}>
                  <Text style={S.actionBtnTxt}>✏️ Edit</Text>
                </TouchableOpacity>
                <TouchableOpacity 
                  style={[S.actionBtn, { backgroundColor: T.roseBg, marginLeft: 8 }]} 
                  onPress={() => Alert.alert('Delete Sale?', 'This will reverse stock and records.', [
                    { text: 'Cancel', style: 'cancel' },
                    { text: 'Delete', style: 'destructive', onPress: () => dispatch(deleteSale(item.id)) },
                  ])}
                >
      ...
      <CreateSaleModal 
        visible={showCreate} 
        onClose={(success) => { setShowCreate(false); if(success) showToast('Sale Created Successfully!'); }} 
      />
      <CreateSaleModal 
        visible={!!editingSale} 
        onClose={(success) => { setEditingSale(null); if(success) showToast('Sale Updated Successfully!'); }} 
