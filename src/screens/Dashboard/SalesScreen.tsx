...
export default function SalesScreen() {
  ...
  const { items, isLoading } = useSelector((state: RootState) => state.sales);
  ...
  useEffect(() => { dispatch(fetchSales(undefined)); }, []);

  const onRefresh = async () => {
    setRefreshing(true);
    await dispatch(fetchSales(undefined));
    setRefreshing(false);
  };

  const handleDelete = (id: string) => {
    Alert.alert('Delete Sale', 'This will reverse stock and payment records.', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => dispatch(deleteSale(id)) },
    ]);
  };

  const handlePdf = (id: string) => {
    apiService.openSalePdf(id);
  };

  const totalRevenue = items.reduce((sum, s) => sum + (s.finalTotal || 0), 0);

  if (isLoading && !items.length) { return <LoadingSpinner message="Loading sales..." />; }
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.summaryRow}>
        ...
        <View style={styles.summaryItem}>
          <Text style={styles.summaryValue}>{formatAmount(totalRevenue)}</Text>
          <Text style={styles.summaryLabel}>Total Revenue</Text>
        </View>
        <TouchableOpacity style={styles.createBtn} onPress={() => setShowCreate(true)}>
          <Text style={styles.createTxt}>+ Create Sale</Text>
        </TouchableOpacity>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={<EmptyState icon="💰" title="No sales yet" subtitle="Tap '+ Create Sale' above to get started" />}
        renderItem={({ item }) => {
              ...
              {isOpen && (
                <View style={styles.details}>
                  ...
                  {(item.items || []).map((si, idx) => (
                    <View key={si.id ?? idx} style={styles.saleItem}>
                      <Text style={styles.siName}>{si.productName}</Text>
                      <Text style={styles.siMeta}>{si.quantity} {si.unitType} × ₹{si.rate} = {formatAmount(si.total)}</Text>
                    </View>
                  ))}
                  ...
                  <View style={styles.actionRow}>
                    <TouchableOpacity
                      ...
                      onPress={() => { setEditingSale(item); setExpanded(null); }}
                    >
...
const styles = StyleSheet.create({
  ...
  summaryRow:    { flexDirection: 'row', backgroundColor: '#22c55e', paddingVertical: 14, paddingHorizontal: 16, alignItems: 'center' },
  ...
  createTxt:     { color: '#ffffff', fontWeight: '800', fontSize: 13 },
  ...
  invoice:       { fontSize: 13, fontWeight: '700', color: '#f16a0a' },
  partyName:     { fontSize: 15, fontWeight: '700', color: '#111827' },
  date:          { fontSize: 12, color: '#9ca3af' },
  amountInfo:    { alignItems: 'flex-end', gap: 6 },
  amount:        { fontSize: 17, fontWeight: '800', color: '#22c55e' },
  ...
  saleItem:      { backgroundColor: '#ffffff', borderRadius: 8, padding: 10, marginBottom: 6 },
  siName:        { fontSize: 14, fontWeight: '600', color: '#1f2937' },
  siMeta:        { fontSize: 12, color: '#9ca3af', marginTop: 2 },

  actionRow:     { flexDirection: 'row', gap: 8, marginTop: 12 },
  editBtn:       { flex: 1, backgroundColor: '#eff6ff', borderRadius: 10, paddingVertical: 10, alignItems: 'center' },
  editTxt:       { fontSize: 13, fontWeight: '700', color: '#1d4ed8' },
